...
@section('developer.roles.all', 'active')

@section('breadcrumb')
    @include('back.partials.breadcrumb',['title' => 'Sistema de Compras', 'active' => 'Todos los Roles', 'breadcrumb' => []])
@endsection
                        ...
                        <table id="petition" class="table table-striped table-bordered table-hover dataTables-example" border="0" cellspacing="0">
                            ...
                            <tbody>
                                @foreach($roles as $item => $value)
                                    <tr>
                                        <td>{{$value->name}}</td>
                                        <td>{{$value->total or '0'}}</td>
                                        <td><a href="{{route('back.security.roles.edit',['id' => $value->id])}}" class="label label-danger">Editar</a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
